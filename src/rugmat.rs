...
impl RugMat {
    ...
    pub fn matmul_vec(&self, v: &[Float]) -> Vec<Float> {
        ...
        let mut result = vec![Float::with_val(precision, 0); self.rows];
...
impl RugMat {
    ...
    fn dot(u: &[Float], v: &[Float], precision: u32) -> Float {
        u.iter().zip(v).map(|(a, b)| a * b).fold(Float::with_val(precision, 0), |acc, x| acc + x)
    }
    ...
    fn householder_vector(x: &[Float], precision: u32) -> Vec<Float> {
        ...
        let sign = if x[0].significant_bits() == 0 || x[0].significant_bits() > 0 && x[0].is_sign_negative() {
        ...
        pub fn full_svd(&self, max_iters: usize) -> SVD {
            ...
            for (i, s) in svals.iter().enumerate() {
                if s.significant_bits() == 0 { continue; } // avoid division by 0
    ...
    pub fn solve_least_squares(&self, b: &[Float], threshold: &Float, max_iters: usize) -> Vec<Float> {
    ...
    fn estimate_condition_number(a: &RugMat, max_iters: usize) -> Float {
        ...
        let inv_ata = ata.pseudoinverse(&Float::with_val(ata.data[0][0].precision(), 1e-50), max_iters);
